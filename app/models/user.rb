class User < ActiveRecord::Base
  attr_accessible :email, :password, :password_confirmation, :city_name, :name
  
  belongs_to :city

  validates_presence_of :email
  validates_presence_of :password, :on => :create, :message => "Password can't be blank!"
  validates_length_of :password, :in => 6..14, :too_short => "Password must beat least 6 characters", 
